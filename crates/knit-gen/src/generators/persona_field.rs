...
impl FieldGenerator for PersonaFieldGenerator {
    fn generate(&self, _rng: &mut dyn RngCore, count: usize, ctx: &GenContext) -> ArrayRef {
        ...
        let trait_values: Vec<Option<&knit_core::Value>> = actor_pks
            .iter()
            .map(|pk_opt| {
                pk_opt
        ...
        match &self.output_data_type {
            DataType::Float64 => {
                let values: Vec<Option<f64>> = trait_values
                    .iter()
                    .map(|v| match v {
                        Some(knit_core::Value::Float(f)) => Some(*f),
                        Some(knit_core::Value::Int(i)) => Some(*i as f64),
                        _ => None,
                    })
                    .collect();
            ...
            DataType::Int64 => {
                let values: Vec<Option<i64>> = trait_values
                    .iter()
                    .map(|v| match v {
                        Some(knit_core::Value::Int(i)) => Some(*i),
                        Some(knit_core::Value::Float(f)) => Some(*f as i64),
                        _ => None,
                    })
                    .collect();
            ...
            _ => {
                ...
                let values: Vec<Option<String>> = trait_values
                    .iter()
                    .map(|v| match v {
                        Some(knit_core::Value::String(s)) => Some(s.clone()),
                        Some(knit_core::Value::Float(f)) => Some(f.to_string()),
                        Some(knit_core::Value::Int(i)) => Some(i.to_string()),
                        Some(knit_core::Value::Bool(b)) => Some(b.to_string()),
                        _ => None,
                    })
                    .collect();
