...
use knit_learn::schema_assembly::{assemble_schema, ColumnAnalysis, TableAnalysis};
...
pub fn run(source: &str, output: &str) -> Result<()> {
    ...
    // 5. Assemble schema
    let schema_text = assemble_schema(&table_analyses);
...
/// contains "id" (case-insensitive) or the cardinality ratio is 1.0.
...
mod tests {
    ...
    #[test]
    fn learn_from_csv_produces_output() {
        ...
        assert!(content.contains("entity"), "schema should contain entity definitions");
        assert!(content.contains("users"), "schema should reference the table name");
    }
