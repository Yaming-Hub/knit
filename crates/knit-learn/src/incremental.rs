...
use crate::fitting::fit_distribution;
use crate::schema_assembly::{ColumnAnalysis, TableAnalysis};
use crate::streaming::state::{ColumnDataType, ColumnState, LearnState, TableState};
use crate::temporal::detect_temporal_pattern;
        ...
        ColumnDataType::String => {
            ...
            if estimated_distinct <= 50 && !items.is_empty() {
                ...
                if !samples.is_empty() {
                    ...
                    let inference = crate::type_inference::infer_type(&sample_opts, 0.3);
