...
fn all_schemas_have_relationships() {
    ...
    for path in &schemas {
        ...
        if stem.contains("cli_test") || stem.contains("nested_objects") || stem.contains("time_series_metrics") || stem.contains("scoped_noise") || stem.contains("event_stream") || stem.contains("sequence_values") {
            continue;
        }
