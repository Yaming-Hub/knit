...
mod tests {
    ...
    fn bool_choices() {
        ...
        assert!(
            true_count > 300 && true_count < 700,
            "expected ~50% true, got {true_count}/1000"
        );
    }
    ...
    fn heavily_skewed_weights() {
        ...
        let choices = vec![
            WeightedChoice {
                value: Value::String("dominant".into()),
                weight: 1000.0,
            },
            WeightedChoice {
                value: Value::String("rare_a".into()),
                weight: 1.0,
            },
            WeightedChoice {
                value: Value::String("rare_b".into()),
                weight: 1.0,
            },
        ];
        let gen = OneOfGenerator::new(choices);
    ...
    fn all_choices_appear_with_equal_weights() {
        ...
        for (k, v) in &counts {
            ...
            assert!(
                ratio > 0.29 && ratio < 0.38,
                "choice '{k}' should be ~33%, got {:.1}%",
                ratio * 100.0
            );
        }
    }
